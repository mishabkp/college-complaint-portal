...
function renderAdminTable(complaints) {
    ...
    complaints.forEach(c => {
        ...
        row.innerHTML = `
            ...
            <td style="padding: 1.25rem;">${c.subject}</td>
            ...
            <td style="padding: 1.25rem;">
                ${c.status !== 'resolved' ?
                `<button class="btn btn-primary" style="padding: 0.4rem 0.8rem; font-size: 0.8rem;" onclick="markResolved('${c.id}')">Mark Resolved</button>` :
                '<span style="color: var(--primary); font-weight: 700;">✓ Fixed</span>'}
                <button class="btn btn-secondary" style="padding: 0.4rem 0.8rem; font-size: 0.8rem; margin-left: 0.5rem;" onclick="deleteComplaint('${c.id}')"><i class="fas fa-trash"></i></button>
            </td>
        `;
